...
function SiteDevices(site){
	...
	if(site.type == 2 || site.type == 3 || site.type == 5 || site.type == 6){
...
Test.getAllZones = function(){
	...
	if(Array.isArray(sites)){
		sites = IX.loop(Test.getCurrentSite(), [], function(acc, site){
			if (site.type !== 1 && site.type !== 4 && site.type !== 5 && site.type !== 6)
				acc.push(site);
			return acc;
		});
...
Test.getSite4WallAndStore = function(){
	var currentSite = Test.getCurrentSite();
	if (IX.isArray(currentSite)){
		return currentSite;
	}else{
		return IX.loop(Test._getAllSites(), [], function(acc, site){
			if (currentSite.type == 5) {
				if (site.id === currentSite.id || site.selected)
					acc.unshift(site);
			} else {
				if (site.id === currentSite.id || site.type == 5 || site.selected)
					acc.unshift(site);
			}
			return acc;
		});
