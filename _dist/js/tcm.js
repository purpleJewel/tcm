...
["siteId", {name : "id", title: "标识号", isLongStr : true}],
["siteType",function(){return getCommonColumnModel("type", "单位类型", function(item){
	return SiteTypes[item.type] || "";
}, true);}],
["siteIP", {name : "ip", title: "IP地址", isLongStr : true}],
["isAffiliateSites", function(){return getCommonColumnModel("selected", "管辖状态", function(item){
	return item.selected ? '<span class="tick"></span>' : '';
}, false);}],
...
function verifyForm(value, msg, name){
	...
	if (name == "id"){
		var re = /^\d{1,2}$/;
		if (!re.test(value)){
			nvAlert("标识号只能是数字且最多两位！");
			return false;
		}
	}
	if (name == "ip" && !value.isIP()){
		if (value === "")
			return true;
		nvAlert(msg);
		return false;
...
'<ul>',
	...
	'<li><span class="label">类型</span>{siteTypeCombo}</li>',
	'<li><span class="label">IP地址</span>',
		'<span><input id="site_ip" maxlength="15" value="{site_ip}"></span></li>',
...
var SiteDialogCfg = {
	...
	tpldataFn : function(rowModel, data){
		...
		if (rowModel)
			data.siteId = '<input id="site_id" class="disabled-id" maxlength="2" disabled value="' + data.site_id + '">';
		else	
			data.siteId = '<input id="site_id" maxlength="2" value="' + data.site_id + '">';
		return data;
	},
	checkers : [
		{name : "id", key: "site_id", empty : "请输入单位标识号！"},
		{name : "no", key: "site_no", empty: "请输入单位编号！"}, 
		{name : "name", key: "site_name", empty : "请输入单位名称！"},
		{name : "type", key: "site_type", empty : ""},
		{name : "ip", key: "site_ip", empty : "请输入单位IP地址,如192.168.1.1！"},
...
}], ["selected.site", function(params, el){
	var ifChecked = $XH.hasClass(el, "selected");
	$XH[ifChecked? "removeClass": "addClass"](el, "selected");
}]]);
...
var t_copy = new IX.ITemplate({tpl: [
	'<div><span class="label">数据源站点IP：</span><input id="copyId" value=""></div>',
'']});

var t_affiliateSites = new IX.ITemplate({tpl: [
	'<tpl id="items">','<div><a class="nv-checkbox {clz}" data-href="$selected.site" data-key="{id}">',
		'<span class="ixpic-"></span><span class="text">{name}</span></a></div>','</tpl>',
'']});
...
TCM.SysDialog.copySites = function(grid){
	showDialog({
		clz: "sys-copy",
		title: "线路及站点信息拷贝",
		content: t_copy.renderData("", {}),
		btns: {
			left : [],
			right : [{name:"copy", text: "确定"}, {name:"cancel", text:"取消"}] 
		},
		listen: {copy: function(cbFn){
			var ip = $X("copyId").value;
			if (!ip.isIP())
				nvAlert("IP输入错误！");
			sysCaller("copySites", {ip: ip}, function(){
				grid.refresh();
				hideDialog();
			}, function(data){
				nvAlert(data.msg);
...
TCM.SysDialog.addAffiliateSites = function(grid){
	var sites = TCM.LineInfo.getSites().getAll();
	var useSites = IX.loop(sites, [], function(acc, site){
		if (site.type === 2  || site.type === 3){
			site.clz = site.selected ? "selected" : "";
			acc.push(site);
		}
		return acc;
	});
	if (useSites.length === 0)
		return nvAlert("当前线路没有可配置站点,无法进行相关操作!");
	showDialog({
		clz: "sys-affiliate",
		title: "管辖车站配置",
		content: t_affiliateSites.renderData("", {
			items: useSites
		}),
		listen: {ok: function(cbFn){
			var ids = IX.map(jQuery('.sys-affiliate a.selected'), function(el){
				return jQuery(el).attr("data-key");
			});
			sysCaller("affiliateSites", {ids: ids.length === 0 ? [-1] : ids}, function(){
				grid.refresh();
				hideDialog();
			}, function(data){
				nvAlert(data.msg);
...
var GridCfgs = {
	line : {
		...
		clz : "sys-line",
		hasCheckbox: true,
		columns: "siteNo,siteId,name,siteType,siteIP,desc",
		buttons : ["create","delete"],
		dataLoader : function(params, _cbFn){
			sysCaller("getLineInfo", {}, function(data){
				var sites = afterLoadLineInfo(data);
				_cbFn({total: sites.length, items: sites});
				if (jQuery(".btn-copy").length === 0 && TCM.Env.canUpdateLineInfo())
					jQuery(".nvgrid-tools>div").append(jQuery('<a class="btn-copy" data-href="$copy.click" data-key="copy"></a>'));
				if (session.getCurrentSiteType() === 5 && jQuery(".affiliate-sites").length === 0)
					jQuery(".nvgrid-tools>div").append(jQuery('<a class="affiliate-sites" data-href="$add.affiliate" data-key="affiliate"></a>'));
			});
		}
	},
	role : {
...
}], ["copy.click", function(params, el){
	TCM.SysDialog.copySites(grid);
}], ["add.affiliate", function(params, el){
	TCM.SysDialog.addAffiliateSites(grid);
}]]);
function showSysLine(cbFn){
	...
	if (session.getCurrentSiteType() === 5)
		GridCfgs.line.columns = "siteNo,siteId,name,siteType,siteIP,isAffiliateSites,desc";
	...
	'<div class="nv-body">',
		...
		'<div class="{cursor}">',
			'<div class="area">',
				...
				'</div> ',
				'<div class="item"> ',
					'<a data-href="$sys.check" data-key="ControlOsdEnable" class="nv-checkbox {osdClz}">',
						'<span class="ixpic- {checkSite}"></span><span>开启云台控制OSD显示</span>',
					'</a>',
				'</div>',
			'</div>',
			'<div class="area">',
...
var SyncDataText = {
	...
	4 : "修改备用控制中心配置数据时，立即向控制中心TVS和下属各车站／车辆段发送配置数据",
	5 : "修改本车站配置数据时，立即向控制中心和本车站TVS发送配置数据",
	6 : "修改本车站配置数据时，立即向控制中心和本车站TVS发送配置数据",
};
...
function getTpldata4Config(){
	...
	return {
		...
		osdClz : getCheckedCl("ControlOsdEnable"),
		syncTxt : SyncDataText[siteType],
...
globalActionConfig([["sys.check", function(params, el){
	...
	if(!ifSelected && !(params.key == "Sync" || params.key == "ControlOsdEnable"))
...
function addZoneOkFn(data, CurrentSite){
	...
	var $firstLi = jQuery(jQuery(".nv-tree>ul>li")[0]);
	if($firstLi.hasClass("none"))
		$firstLi.removeClass('none').addClass('expand');
	var ul = jQuery(".nv-tree>ul>li:first>ul")[0];
	if(!ul){
		ul = jQuery("<ul></ul>");
		$firstLi.append(ul);
	}
...
function createZoneGrid(treeCfg){
	...
	return new TCM.Lib.Grid($XP(treeCfg, "container", 'Grid'),{
		...
		grid : {
			...
			dataLoader : function(params, _cbFn){
				deviceCaller("getCamerasByZone", IX.inherit(params, {
					...
					resizeGridFn(!isOCC || !policeStation);
					$Xw.bind({"resize": function(){
						resizeGridFn(!isOCC || !policeStation);
					}});
				});
		...
		tools : {
			buttons: isOCC || !isHandle ? ["refresh"] : ["refresh", "create", "delete"]
		},
		listen : isOCC || !isHandle ? {} : {
			createItem : createFn,
...
globalActionConfig([
	...
	["show.map", function(params, el, evt){
		...
		var siteId = $XD.dataAttr(el, "siteid");
		var mapHandle = siteId == CurrentSite.id ? false : true;
		if(!zoneId)
...
function showZoneList(cbFn){
	getCamerasZone(function(_cfg){
		var cfg = IX.inherit(_cfg, {
			htmlFn : function(node) {
				var deleteZone = node.siteId != CurrentSite.id ? "" :
		...
		if(span.length === 0){
			zoneTreeClick({keys: "20,21,22,31,32,33",name: "", zoneId: "", siteId : jQuery(".nv-tree>ul>li>a:first").attr("data-siteid")});
		}else{
			span.click();
			if(!(isOCC || policeStation))
...
function getOCCTreeHTML(sites){
	var nodes = IX.map(sites, function(site, idx){
...
TCM.Device.init = function(pageCfg, pageParams, cbFn){
	...
	policeStation =  (currentSiteType == 5 || currentSiteType == 6) ? true : false;
	switch(pageCfg.name){
	case "device-list":
		showDeviceList(cbFn);
		break;
